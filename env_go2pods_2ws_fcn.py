...
class Maze(tk.Tk, object):
    ...
    def get_state(self, agent_id=(config.n_agv - 1)):
        ...
        is_loaded = True if agent_id in self.loaded_agv else False

        x_direction = agent_pos_x - task_pos_x
        y_direction = agent_pos_y - task_pos_y
        xy_distance = (x_direction**2 + y_direction**2)**(1 / 2.0)
        self.state_fcn = np.array([agent_pos_x, agent_pos_y, task_pos_x, task_pos_y, x_direction, y_direction, xy_distance, is_loaded])#,
