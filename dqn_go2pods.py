...
class DeepQNetwork_cnn(nn.Module):
    def __init__(self, lr, input_dims, fc1_dims, fc2_dims, n_actions, bias=True):
        ...
        # convolution layers
        self.conv_1  = nn.Conv2d(2, 32, kernel_size=3, stride=1)
...
class Agent:
    def __init__(self, gamma, epsilon, lr, input_dims, batch_size, n_actions, c_step=300,
        ...
        self.state_memory      = np.zeros((self.mem_size, 2, *input_dims), dtype=np.float32)
        self.state2_memory     = np.zeros((self.mem_size, 7)             , dtype=np.int32)
        self.action_memory     = np.zeros(self.mem_size                  , dtype=np.int32)
        self.reward_memory     = np.zeros(self.mem_size                  , dtype=np.float32)
        self.new_state_memory  = np.zeros((self.mem_size, 2, *input_dims), dtype=np.float32)
